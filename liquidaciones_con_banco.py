...
# ========= Reglas de casos =========
# (mismos diccionarios que tu app original)
...
if generate:
    ...
    if case_choice == "Todos":
        ...
        # Guardamos todas las liquidaciones concatenadas para conciliación
...
if bank_file:
    try:
        # header es 0-based; usuario entra 1-based
...
if raw_bank is not None:
    ...
    # Filtro útil: solo salidas (pagos)
    ...
    if btn_reconcile:
        ...
        # Campos mínimos
        reqs = ["Pago al propietario"]
        if liquid_date_col in liq.columns:
            pass
        else:
        ...
        # Índice por importe para búsqueda rápida
        ...
        for j, rb in bank2.iterrows():
            ...
            # Filtrar candidatos no usados y con fecha cercana
        ...
        # No conciliados
        matched_bank_idx = set(df_matches.index)  # indices en df_matches, no en bank; rehacemos
        # reconstruimos usando fechas e importes para marcar
        key_bank = set()
        for _, r in df_matches.iterrows():
            key_bank.add( (r["Fecha mov."], round(abs(r["Importe mov."]),2)) )
        bank_keys = set( (r["__Fecha__"].date(), round(abs(r["Importe"]),2)) for _, r in bank2.iterrows() )
        liq_keys_used = set()
        for _, r in df_matches.iterrows():
            liq_keys_used.add( (pd.to_datetime(r["Fecha entrada"]).date() if not pd.isna(r["Fecha entrada"]) else None,
                                round(abs(r["Pago al propietario"]),2)) )

        unmatched_bank = bank2[[ "Fecha","Concepto","Beneficiario/Ordenante","Importe"]].copy()
        unmatched_bank["abs_key"] = list((unmatched_bank["Fecha"].dt.date, unmatched_bank["Importe"].abs().round(2)))
        unmatched_bank = unmatched_bank[~unmatched_bank.apply(lambda x: (x["Fecha"].date(), round(abs(x["Importe"]),2)) in key_bank, axis=1)].drop(columns=["abs_key"])

        liq2 = liq.copy()
        liq2["abs_key"] = list((liq2["__FechaRef__"].dt.date, liq2["__Pago__"].abs().round(2)))
        unmatched_liq = liq2[~liq2.apply(lambda x: (x["__FechaRef__"].date() if not pd.isna(x["__FechaRef__"]) else None, round(abs(x["__Pago__"]),2)) in liq_keys_used, axis=1)]
