...
if raw_bank is not None:
    ...
    if btn_reconcile:
        ...
        # 2) 1↔N (pago cubre varias reservas) con prioridad: (portal+base) → (portal) → global
        if enable_1N:
            for j, rb in bank2.iterrows():
                if j in used_bank: continue
                b_amt_c = int(rb["__ImporteAbsCents__"])
                ...
                cands["__d_dias__"] = (cands["__FechaRef__"] - rb["__Fecha__"]).abs().dt.days
                cands = cands.sort_values("__d_dias__").head(80)

                tried = False
                # (a) Portal + Base
                if restrict_same_portal and group_by_base_aloj and "__Portal__" in cands.columns and "__AlojBase__" in cands.columns:
                    tried = True
                    for p in cands["__Portal__"].dropna().unique():
                        cands_p = cands[cands["__Portal__"] == p]
                        for base in cands_p["__AlojBase__"].dropna().unique():
                            ...
                            if sol:
                                ...
                                break
                        if j in used_bank: break

                # (b) Solo Portal
                if restrict_same_portal and not (j in used_bank):
                    tried = True
                    for p in cands["__Portal__"].dropna().unique():
                        sub = cands[cands["__Portal__"] == p]
                ...
                # (c) Global
                if not (j in used_bank):
                    # si no hemos probado nada aún o no hubo solución
                    sub = cands
                    if len(sub) >= 2:
                        vals_c = sub["__PagoCents__"].abs().astype(int).tolist()
                        idxs = sub.index.tolist()
        ...
        # 3) N↔1 (varios pagos cubren una reserva) con céntimos
        if enable_N1:
