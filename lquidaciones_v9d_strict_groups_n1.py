...
if raw_bank is not None:
    ...
    if btn_reconcile:
        ...
        # Detectar base en texto del banco (Concepto/Beneficiario) para evitar mezclar bases en N↔1
        known_bases = sorted([b for b in liq['__AlojBase__'].dropna().unique().tolist() if isinstance(b, str) and b.strip()],
                             key=lambda x: -len(x))  # más largas primero

        def detect_base_in_text(txt: str) -> str | None:
