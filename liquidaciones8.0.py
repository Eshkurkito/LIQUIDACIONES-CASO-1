...
def fmt_number_for_ui(colname: str, x):
    ...
    if colname.strip().lower() == NIGHTS_COL.lower():
        try:
            v = float(x)
            return f"{int(round(v))}"
        except Exception:
            return x
    # Monetario -> 2 decimales + €
    if colname in MONEY_COLS_CANON:
        try:
            s = f"{float(x):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            return f"{s} €"
        except Exception:
            return x
    # Otro numérico -> 2 decimales
    try:
...
def show_table_es(df, title):
    st.subheader(title)
    view = ensure_unique_columns(df).copy()
    for c in view.columns:
        if pd.api.types.is_numeric_dtype(view[c]) or c in MONEY_COLS_CANON or c == NIGHTS_COL:
            view[c] = view[c].apply(lambda v: fmt_number_for_ui(c, v))
...
processors = {
    1: process_case1, 2: process_case2, 3: process_case3, 4: process_case4, 5: process_case5
}

# ========= Exportación Excel formateado =========
...
def write_grouped_sheet(ws, df):
    ...
    def is_money(colname): return colname in MONEY_COLS_CANON
    def is_nights(colname): return colname.strip().lower() == NIGHTS_COL.lower()
    ...
    def write_table(start_row, subdf):
        ...
        # Sumatorios debajo (negrita en numéricos)
...
st.caption("Sube el Excel, elige período y caso (o Todos), y pulsa Generar. La imputación es por Fecha de entrada (sin prorrateo).")
...
if generate:
    ...
    # Func para ejecutar un caso y postformatear
    def run_case(case_no):
        ...
        # Asegurar redondeos: noches int, resto 2 decimales
    ...
    if case_choice == "Todos":
        ...
        for c in [1,2,3,4,5]:
            ...
            total_warns += warn
            dfs[f"Caso {c}"] = df_out.sort_values(by=[col for col in ["Alojamiento","Fecha entrada"] if col in df_out.columns])
        st.success(f"Liquidación generada (Todos) • {start_date.strftime('%d/%m/%Y')}–{end_date.strftime('%d/%m/%Y')}")
        for label, df_show in dfs.items():
            show_table_es(df_show, f"{label} — Tabla de liquidaciones")
        # Excel multihoja
        ...
        show_table_es(df_out, "Tabla de liquidaciones")
        ...
        if aloj_col is not None and pago_col is not None:
            ...
            st.markdown(f"**Total general a transferir:** {fmt_number_for_ui('Pago al propietario', total_general)}")
