...
st.set_page_config(page_title="LIQUIDACIONES (Casos 1–5) + Conciliación bancaria avanzada", page_icon="🏦", layout="wide")
...
def apply_commission_vat_by_scope(df: pd.DataFrame, vat_pct: float, treat_empty_as_booking: bool, skip_booking_vat: bool,
    ...
    """Aplica IVA sobre 'Comisión portal' SOLO si Portal contiene 'booking' (y opcionalmente portal vacío),
    además de un 'scope_mask' extra para acotar por caso (p.ej., solo APOLO 29/197 en Caso 2).
    vat_pct se pasa como porcentaje: 21 -> 21%.
    Devuelve (df_out, warn_count_portal_vacio_con_comision).
    """
    out = df.copy()
...
def process_case1(df, treat_empty_as_booking=False, skip_booking_vat=False, vat_pct=21.0):
    ...
    scope = pd.Series(True, index=df.index)  # todos los alojamientos del caso
...
def process_case4(df, treat_empty_as_booking=False, skip_booking_vat=False, vat_pct=0.0):
    ...
    # Aunque vat_pct por defecto es 0 en caso 4, dejamos la llamada para permitir override desde UI
    scope = pd.Series(True, index=df.index)
    df, warn_count = apply_commission_vat_by_scope(df, vat_pct, treat_empty_as_booking, skip_booking_vat, scope)
...
if raw_bank is not None:
    ...
    with coly1:
        restrict_same_portal = st.checkbox("Restringir combinaciones a mismo Portal", value=True)
    with coly2:
        group_by_base_aloj = st.checkbox("Agrupar por Alojamiento base (quita números)", value=True)
    ...
    def alojamiento_base(s: str) -> str:
        s = (s or "").upper()
        # Quitar números y guiones/espacios múltiples para obtener un "prefijo" base
        s = re.sub(r"\b\d+\b", "", s)
        s = re.sub(r"\s{2,}", " ", s)
        return s.strip()

    def subset_sum_indices(values, indices, target, tol, kmax):
        """
        Backtracking con poda: busca subconjunto (hasta kmax) cuya suma ≈ target dentro de tol.
        values: lista de importes (positivos)
        indices: índices asociados a cada valor
        """
        best = None
        n = len(values)
        order = sorted(range(n), key=lambda i: abs(values[i]-target))  # ordenar por cercanía
        ...
        def backtrack(start, kleft, acc_sum, chosen):
            ...
            # poda por tamaño
            if kleft == 0:
                if abs(acc_sum - target) <= tol + 1e-9:
                    best = chosen[:]
                return
            # poda por solución ya encontrada
            if best is not None:
                return
            for p in range(start, len(vals)):
                v = vals[p]
                # poda simple: si excede demasiado y todos son positivos, pero dejamos margen por tol
                if acc_sum + v - target > tol + 1.0:  # +1 amortigua redondeos
                    continue
    ...
    if btn_reconcile:
        ...
        # 1) 1↔1 básico con tolerancias
        for j, rb in bank2.iterrows():
            if j in used_bank: continue
            b_amt = float(abs(rb["__ImporteAbs__"]))
            ...
            cands = cands.sort_values(["__diff__", "__d_dias__"])
            i = cands.index[0]
            ...
            matches_rows.append({
                ...
                "Δ importe": round(float(abs(abs(rf["__Pago__"]) - b_amt)),2)
            })
        ...
        def candidate_liq_for_bank(j, rb):
            ...
            if restrict_same_portal and "__Portal__" in cands.columns:
                # agrupar luego, pero para subset exigiremos que todos compartan el mismo portal
                pass
            if group_by_base_aloj and "__AlojBase__" in cands.columns:
                pass
            return cands

        # 2) 1↔N (pago cubre varias reservas)
        if enable_1N:
            for j, rb in bank2.iterrows():
                if j in used_bank: continue
                b_amt = float(abs(rb["__ImporteAbs__"]))
                cands = candidate_liq_for_bank(j, rb)
                if cands.empty: 
                    continue
                # Orden preliminar por cercanía de importe y fecha
                cands["__d_importe__"] = (cands["__Pago__"].abs() - b_amt).abs()
                cands["__d_dias__"] = (cands["__FechaRef__"] - rb["__Fecha__"]).abs().dt.days
                cands = cands.sort_values(["__d_dias__", "__d_importe__"]).head(50)  # reducir universo

                # Si hay restricción de portal, construimos subconjuntos dentro de cada portal
                portals = [None]
                if restrict_same_portal:
                    portals = list(cands["__Portal__"].dropna().unique())
                for p in portals:
                    cands_p = cands if p is None else cands[cands["__Portal__"] == p]
                    if cands_p.empty: 
                        continue
                    # Restricción de alojamiento base (todos mismo base)
                    if group_by_base_aloj:
                        for base in cands_p["__AlojBase__"].dropna().unique():
                            sub = cands_p[cands_p["__AlojBase__"] == base]
                            if len(sub) < 2: 
                                continue
                            vals = sub["__Pago__"].abs().astype(float).tolist()
                            idxs = sub.index.tolist()
                            sol = subset_sum_indices(vals, idxs, b_amt, float(amt_tol), int(max_k))
                            if sol:
                                # Registrar match 1↔N
                                used_bank.add(j)
                                for i in sol: used_liq.add(i)
                                subsel = liq.loc[sol]
                                ...
                                matches_rows.append({
                                    ...
                                    "Portal": p if p is not None else "",
                                    ...
                                    "Δ importe": round(abs(pago_sum - b_amt),2)
                                })
                                break  # no buscar más en este movimiento
                    else:
                        # Sin restricción de base, usar todos cands_p
                        if len(cands_p) >= 2:
                            vals = cands_p["__Pago__"].abs().astype(float).tolist()
                            idxs = cands_p.index.tolist()
                            sol = subset_sum_indices(vals, idxs, b_amt, float(amt_tol), int(max_k))
                            if sol:
                                used_bank.add(j)
                                for i in sol: used_liq.add(i)
        ...
        # 3) N↔1 (varios pagos cubren una reserva)
        if enable_N1:
            # Precalcular banco sin usar
            for i, rf in liq.loc[~liq.index.isin(used_liq)].iterrows():
                l_amt = float(abs(rf["__Pago__"]))
                # movimientos en ventana
                ...
                if len(bsub) < 2: 
                    continue
                # Orden preliminar
                bsub["__d_importe__"] = (bsub["__ImporteAbs__"].abs() - l_amt).abs()
                bsub["__d_dias__"] = (bsub["__Fecha__"] - rf["__FechaRef__"]).abs().dt.days if pd.notna(rf["__FechaRef__"]) else 0
                bsub = bsub.sort_values(["__d_dias__", "__d_importe__"]).head(50)
                vals = bsub["__ImporteAbs__"].abs().astype(float).tolist()
                idxs = bsub.index.tolist()
                sol = subset_sum_indices(vals, idxs, l_amt, float(amt_tol), int(max_k))
                if sol:
                    used_liq.add(i)
                    ...
                    matches_rows.append({
                        ...
                        "Δ importe": round(abs(pago_sum - l_amt),2)
                    })
