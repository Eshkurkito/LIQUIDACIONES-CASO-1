...
# --- PATCH2: apply_booking_vat_on_commission robusta ---

def apply_booking_vat_on_commission(df, portal_col="Portal", commission_col="Comisión portal"):
    out = df.copy()

    # Candidatos alternativos por si llegan con otros nombres
    ...
    # Resolver nombre real existente
    ...
    if not port_name or not comm_name:
        return out  # no hay columnas suficientes, no hacemos nada

    # Extraer serie de portal aún si hay duplicados
    ser = out[port_name]
    ...
    # Forzar a texto seguro y detectar booking
    ser = ser.astype("string").fillna("")
    mask = ser.str.lower().str.contains("booking", na=False)

    # Asegurar comisión numérica antes de multiplicar
...
def apply_booking_vat_on_commission(df, portal_col="Portal", commission_col="Comisión portal"):
    ...
    if not port_name or not comm_name:
        return out  # no hay columnas suficientes
