...
if raw_bank is not None:
    ...
    if btn_reconcile:
        ...
        used_bank_idx = set()
        matches = []
        amt_eps = float(amt_tol)
        for j, rb in bank2.iterrows():
            b_amt = float(abs(rb["__ImporteAbs__"]))
            best = None; best_days = None; best_diff = None
            for i, rf in liq.dropna(subset=["__Pago__"]).iterrows():
                if i in used_liq:
                    continue
                l_amt = float(abs(rf["__Pago__"]))
                diff = abs(b_amt - l_amt)
                if diff <= amt_eps + 1e-9:
                    f_liq = rf["__FechaRef__"]; f_bnk = rb["__Fecha__"]
                    if pd.isna(f_liq) or pd.isna(f_bnk):
                        continue
                    d = abs((f_liq - f_bnk).days)
                    if d <= int(days_tol):
                        if (best is None) or (diff < best_diff - 1e-9) or (abs(diff - best_diff) <= 1e-9 and d < best_days):
                            best = i; best_days = d; best_diff = diff
            if best is not None:
                used_liq.add(best)
                used_bank_idx.add(j)
                rf = liq.loc[best]
                matches.append({
                    ...
                    "Δ días": best_days,
                    "Δ importe": round(best_diff, 2) if best_diff is not None else None
                })

        df_matches = pd.DataFrame(matches)

        # Banco sin match por índice
        unmatched_bank = bank2.loc[~bank2.index.isin(used_bank_idx), ["Fecha","Concepto","Beneficiario/Ordenante","Importe"]].copy()

        # Liquidaciones sin match por índice
