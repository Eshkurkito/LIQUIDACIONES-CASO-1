...
def ensure_required(df, required, ctx=""):
    ...
    if missing:
        st.error(f"Faltan columnas requeridas: {missing} {('en ' + ctx) if ctx else ''}. "
                 f"Comprueba el Excel (recuerda que en Avantio el encabezado real suele estar en la fila 2).")
        st.stop()
...
def normalize_columns(df):
    ...
    # Red de seguridad: si Alojamiento no se detectó, intenta columnas que contengan 'aloj'
...
# ----------------------
# Procesadores por caso (robustos con row.get)
# ----------------------
def process_case1(df):
    ...
    def honorarios(row):
        key = str(row.get("Alojamiento","")).strip().upper()
        pct = case1_percent_amenities.get(key, (0.20, 0.0))[0]
        return float(row.get("Ingreso alojamiento", 0.0)) * pct * 1.21
...
def process_case2(df):
    ...
    def honorarios(row):
        ...
        base = float(row.get("Ingreso alojamiento", 0.0)) - float(row.get("IVA del alquiler", 0.0))
...
def process_case3(df):
    ...
    def honorarios(row):
        ...
        base = float(row.get("Ingreso alojamiento", 0.0)) - float(row.get("Comisión portal", 0.0))
        return base * pct * 1.21
...
def process_case4(df):
    ...
    def honorarios(row):
        base = float(row.get("Ingreso alojamiento", 0.0)) - float(row.get("IVA del alquiler", 0.0)) - float(row.get("Comisión portal", 0.0))
        return base * 0.20
