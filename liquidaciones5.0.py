...
def process_case2(df, treat_empty_as_booking=False):
    ...
    # Ajuste general de Booking + opción vacíos
    df, warn_count = adjust_commission_booking_and_empty_portal(df, treat_empty_as_booking)
...
def process_case4(df, treat_empty_as_booking=False):
    ...
    # Para coherencia, también ajustamos comisión (aunque normalmente no hay +21% salvo Booking)
    df["Portal"] = df.get("Portal", pd.Series([""]*len(df)))  # asegura Portal
    df, warn_count = adjust_commission_booking_and_empty_portal(df, treat_empty_as_booking)
    ...
    # ✅ Devolvemos columnas asegurando Alojamiento y Pago al propietario
    cols = ["Alojamiento","Fecha entrada","Fecha salida","Noches ocupadas","Ingreso alojamiento","IVA del alquiler",
            "Portal","Comisión portal","Honorarios Florit","Gasto limpieza","Amenities","Total Gastos",
            "Pago al propietario","Pago recibido","Ingreso limpieza"]
    cols = [c for c in cols if c in out.columns]
    return out[cols].sort_values(by=["Alojamiento","Fecha entrada"] if "Fecha entrada" in cols else ["Alojamiento"]), warn_count
...
st.caption("Sube el Excel, elige período y caso, y pulsa Generar en la izquierda. Se mostrará directamente la liquidación.")
...
if generate:
    ...
    # Aviso de portal vacío con comisión > 0
