...
def adjust_commission_booking_and_empty_portal(df, treat_empty_as_booking=False):
    ...
    num_warn = número de filas con portal vacío y comisión > 0 (para warning).
    """
    out = df.copy()
    portal_col = "Portal"
...
def process_case1(df, treat_empty_as_booking=False):
    ...
    return out[cols].sort_values(by=["Alojamiento","Fecha entrada"] if "Fecha entrada" in cols else ["Alojamiento"]), warn_count
...
def process_case4(df, treat_empty_as_booking=False):
    ...
    out["Gasto limpieza"]   = out.get("Ingreso limpieza", 0.0)
    out["Amenities"]        = 0.0
    # Asegurar "IVA del alquiler" en salida (si no viene, 0.00)
    if "IVA del alquiler" not in out.columns:
        out["IVA del alquiler"] = 0.0
    out["Total Gastos"]     = out[["Comisión portal","Honorarios Florit"]].sum(axis=1) + out["Gasto limpieza"] + out["Amenities"]
    out["Pago al propietario"] = out["Ingreso alojamiento"] - out["IVA del alquiler"] - out["Comisión portal"] - out["Honorarios Florit"]
...
def write_grouped_sheet(workbook, df, title="Liquidación"):
    ...
    # Columnas a exportar (en el orden final del df)
    cols = list(df.columns)

    # Función para escribir una tabla (cabecera + filas) con bordes y número español
    def write_table(start_row, subdf):
        ...
        for j, col in enumerate(cols, start=1):
            if j == 1:  # ya escrito "TOTAL"
                continue
            # Sumar si es numérico
            if pd.api.types.is_numeric_dtype(subdf[col]):
                ...
                c.border = BORDER_THIN
        return sum_row + 2  # siguiente bloque deja una fila en blanco
...
# ========= UI (sin vista previa, botón en el menú) =========
st.title("📊 LIQUIDACIONES Automáticas (Casos 1–5) + Export Excel")
st.caption("Sube el Excel, elige período y caso, y pulsa Generar en la izquierda. Se mostrará directamente la liquidación y podrás exportar a Excel.")
...
with st.sidebar:
    ...
    st.checkbox("Prorratear por noches del período", value=True, key="prorate")
...
if generate:
    ...
    # Leer y asegurar columnas únicas
    df_in = pd.read_excel(file, header=0)
    df_in = ensure_unique_columns(df_in)

    # Normalizar (por nombre o por letras) y asegurar único
    ...
    # Procesar caso + avisos portal vacío
    ...
    if warn_count > 0 and not treat_empty_as_booking:
        st.warning(f"Hay {warn_count} reservas con comisión > 0 pero portal vacío. "
                   f"Si deben ser Booking, marca la opción «Tratar reservas sin portal como Booking (+21% comisión)» y vuelve a generar.")

    # Aplicar periodo/prorrateo y asegurar único
    ...
    # Resumen por alojamiento robusto (incluye sufijos y recálculo caso 4 si faltase)
    aloj_col = find_col(result_df, "Alojamiento")
    pago_col = find_col(result_df, "Pago al propietario")
    if pago_col is None and case_choice == 4:
        ing_col   = find_col(result_df, "Ingreso alojamiento")
        iva_col   = find_col(result_df, "IVA del alquiler")
        comi_col  = find_col(result_df, "Comisión portal")
        hono_col  = find_col(result_df, "Honorarios Florit")
        if ing_col and comi_col and hono_col:
            base_ing = pd.to_numeric(result_df[ing_col], errors="coerce").fillna(0.0)
            base_iva = pd.to_numeric(result_df[iva_col], errors="coerce").fillna(0.0) if iva_col else 0.0
            base_com = pd.to_numeric(result_df[comi_col], errors="coerce").fillna(0.0)
            base_hon = pd.to_numeric(result_df[hono_col], errors="coerce").fillna(0.0)
            result_df["Pago al propietario (recalc)"] = (base_ing - base_iva - base_com - base_hon).round(2)
            pago_col = "Pago al propietario (recalc)"

    if aloj_col is not None and pago_col is not None:
        pagos = (result_df[[aloj_col, pago_col]]
