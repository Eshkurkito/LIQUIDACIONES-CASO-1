...
def apply_booking_vat_on_commission(df, portal_col="Portal", commission_col="Comisión portal"):
    ...
    portal_candidates = [portal_col, "Portal", "Web origen", "Canal", "Fuente"]
    commission_candidates = [commission_col, "Comisión portal", "Comisión Portal/Intermediario: Comisión calculada", "Comisión"]
    port_name = next((c for c in portal_candidates if c in out.columns), None)
    comm_name = next((c for c in commission_candidates if c in out.columns), None)
    if not port_name or not comm_name:
        return out

    ser = out[port_name]
    if isinstance(ser, pd.DataFrame):
        ser = ser.iloc[:, 0]
    elif not isinstance(ser, pd.Series):
        ser = pd.Series(ser)
    ser = ser.astype("string").fillna("")
    mask = ser.str.lower().str.contains("booking", na=False)

    out[comm_name] = pd.to_numeric(out[comm_name], errors="coerce").fillna(0.0)
    out.loc[mask, comm_name] = out.loc[mask, comm_name] * 1.21
    return out
...
def to_excel_grouped(df, group_col="Alojamiento", name="Liquidación"):
    ...
    for g, sub in df.groupby(group_col):
        for j, cn in enumerate(cols, start=1):
            c = ws.cell(row=row_cursor, column=j, value=cn); c.font=bold; c.border=border; c.alignment=center
        row_cursor += 1
        for _, r in sub.iterrows():
            for j, cn in enumerate(cols, start=1):
                c = ws.cell(row=row_cursor, column=j, value=r[cn]); c.border=border
            row_cursor += 1
        for j, cn in enumerate(cols, start=1):
            if cn == group_col:
                ...
                c = ws.cell(row=row_cursor, column=j, value=val); c.font=bold; c.border=border
        row_cursor += 2
...
case5_percent_amenities = {"HOMERO 01": (0.20, 0.00), "HOMERO 02": (0.20, 0.00),
...
def detect_case(df):
    ...
    scores = {1: len(alos & set(case1_props)), 2: len(alos & set(case2_props)), 3: len(alos & set(case3_props)), 4: len(alos & set(case4_props)), 5: len(alos & set(case5_props))}
...
st.caption("Sube Excel de Avantio (.xlsx) • Detecta caso • Período y prorrateo • Exporta Excel • Vista para copiar")
...
if file is not None:
    ...
    if df_in.columns.duplicated().any():
        st.warning("Había columnas duplicadas en el Excel. He dejado sólo la primera aparición de cada nombre.")
    ...
    if st.button("Generar liquidación"):
        ...
        # --------- VISTA PARA COPIAR ---------
        st.subheader("📋 Tabla final para copiar (formato español)")
        def fmt_es(x):
            if isinstance(x, (int, float, np.floating)):
                return f"{float(x):,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            return x
        vista_copia = final_df.copy()
        for c in vista_copia.columns:
            if pd.api.types.is_numeric_dtype(vista_copia[c]):
                vista_copia[c] = vista_copia[c].apply(fmt_es)
        st.dataframe(vista_copia, use_container_width=True)

        if "Alojamiento" in final_df.columns and "Pago al propietario" in final_df.columns:
            st.subheader("💸 Lista de pagos por apartamento (suma)")
            pagos = (final_df.groupby("Alojamiento", as_index=False)["Pago al propietario"]
                     .sum().round(2).sort_values("Alojamiento"))
            st.dataframe(pagos, use_container_width=True)
            csv_text = pagos.to_csv(index=False, sep=";")
            st.text_area("Copiar lista (CSV con ';')", csv_text, height=180)
            total_general = pagos["Pago al propietario"].sum()
            st.markdown(f"**Total general a transferir:** {fmt_es(total_general)} €")
